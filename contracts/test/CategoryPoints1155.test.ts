...
describe('CategoryPoints1155', () => {
  ...
  describe('Deployment & Roles', () => {
    it('should assign DEFAULT_ADMIN_ROLE to deployer', async () => {
      void expect(await cp.hasRole(await cp.DEFAULT_ADMIN_ROLE(), admin.address)).to.be.true;
    });

    it('should allow admin to grant MINTER_ROLE', async () => {
      void expect(await cp.hasRole(await cp.MINTER_ROLE(), minter.address)).to.be.true;
    });
  ...
  describe('Minting', () => {
    ...
    it('should revert if non-minter tries to mint', async () => {
      await expect(
        cp.connect(user).mint(user.address, CATEGORY_GIYIM, 100n),
    ...
    it('should revert batchMint with mismatched lengths', async () => {
      await expect(
        cp.connect(minter).batchMint(user.address, [CATEGORY_GIYIM], [100n, 200n]),
  ...
  describe('Burning', () => {
    ...
    it('should revert if non-burner tries to burn', async () => {
      await expect(
        cp.connect(user).burn(user.address, CATEGORY_GIYIM, 50n),
      ).to.be.revertedWithCustomError(cp, 'AccessControlUnauthorizedAccount');
    });
  });
  ...
  describe('Transfer Lock (P2P Disabled)', () => {
    ...
    it('should REVERT user-to-exchange transfer', async () => {
      await expect(
        cp
          .connect(user)
          .safeTransferFrom(user.address, exchange.address, CATEGORY_GIYIM, 10n, '0x'),
      ).to.be.revertedWith('CP: P2P transfer disabled');
    });
    ...
    it('should ALLOW internal routing: exchange -> taskRewardManager', async () => {
      ...
      await cp
        .connect(exchange)
        .safeTransferFrom(exchange.address, taskRewardManager.address, CATEGORY_GIYIM, 20n, '0x');
    ...
    it('should ALLOW internal routing: taskRewardManager -> exchange', async () => {
      ...
      await cp
        .connect(taskRewardManager)
        .safeTransferFrom(taskRewardManager.address, exchange.address, CATEGORY_GIYIM, 20n, '0x');
  ...
  describe('Pause', () => {
    it('should allow PAUSER_ROLE to pause and unpause', async () => {
      ...
      void expect(await cp.paused()).to.be.true;

      await cp.connect(admin).unpause();
      void expect(await cp.paused()).to.be.false;
    });

    it('should revert mint while paused', async () => {
      await cp.connect(admin).pause();
      await expect(
        cp.connect(minter).mint(user.address, CATEGORY_GIYIM, 100n),
